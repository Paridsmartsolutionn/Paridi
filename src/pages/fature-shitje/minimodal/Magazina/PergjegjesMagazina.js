...
import PozicioniMagazina from './PozicioniMagazina';
...
const PergjegjesMagazina = ({fetchPrgjMag}) => {     
...
const[testim,setTestim]=useState([])
    ...
    const fetchPergjegjesMag = async () => {
        try {
...
<ButtonGroup size="xl" className="mb-2">
    ...
    <Button className='p-2' onClick={(e)=>{e.preventDefault();submitHandler();setState(defaultState)}} disabled={disabled} type="submit"><AppRegistrationIcon/>Rregjistrim</Button>
</ButtonGroup>
</div>
...
<div className='flex justify-start items-center'>
    <KategoriSigurim fetchKategoriSigurim={fetchPergjegjesMag}/>
    <Form.Select
...
<div className='flex justify-start items-center'>
<PozicioniMagazina fetchPozicioniMagazina ={fetchPergjegjesMag}/>
    <Form.Select
