...
const Qytet = ({ fetchUpdate, qytetet, setQytetet }) => {
  const defaultState = {
    Kodi: "",
    Pershkrim: "",
  };
  ...
  const deleteRow = (row) => {
    let filteredRows = [...qytetet];
    filteredRows = filteredRows.filter((p) => p.Kodi !== row.Kodi);

    setQytetet(filteredRows);
  };
