...
const PrimeGrid = ({
  ...
  const filterdColumns=showHideColumns ? columns?.filter(column=>selectedColumns?.some((field) => field === column?.field)) : columns
  ...
  const dynamicColumns = filterdColumns?.map(
    (col, i) => {
      return (
        <Column
          // filter
...
const formatCurrency = (value) => {
  return value.toLocaleString('en-US', {style: 'currency', currency: 'ALL'});
}
...
const footerGroup=()=>{
  ...
  let checkCols=filterdColumns?.some(column=>{totalColSpans++; return column?.allowSum})
  if(!checkCols) return <></>
  ...
  <Row>
      {filterdColumns?.map(column=>{
          ...
          return <Column  key={column?.field} colSpan={1} footer={()=>columnSum(column?.field)} />
                  }
...
const menuModel = [
  ...
  {label: 'Fshije', icon: 'pi pi-fw pi-times', command: () => deleteRow(selectedProduct)},
  {label: 'ExportCSV',  icon:"pi pi-file",  command: () => exportCSV(false)},
  {label: 'ExportExel',  icon:"pi pi-file-excel",  command: () => exportExcel()},
...
const deleteRow = (row) => {
  ...
  toast.current.show({severity: 'error', summary: 'Rrjeshti u fshi', detail: row.field});
...
const exportColumns = columns.map(col => ({ title: col.title, dataKey: col.field }));
