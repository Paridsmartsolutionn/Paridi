...
const PrintoPdf = ({rows,state,adresa1,nipt1}) => {
  ...
  const dynamicColumns = colonatEprintimit.map((col,i) => {
    return <Column key={col.field} field={col.field} header={col.title.split("_").join(" ")} />;
});


const total = rows?.map(total => {return total.Total})
const TvshVlera = rows?.map(tvshvlere => {return tvshvlere.Tvsh_Vlera})
const skontoVlera = rows?.map(skontovlere => {return skontovlere.Skonto_Vlera})
const vleraPaTvsh = rows?.map(vlerepaskonto => {return vlerepaskonto.Vlera_Pa_Tvsh})

 let newTot =  total.reduce(function(index, val) { return index + val; }, 0)
 let newTvshVlera =  TvshVlera.reduce(function(index, val) { return index + val; }, 0)
 let newskontoVlera =  skontoVlera.reduce(function(index, val) { return index + val; }, 0)
 let newvleraPaTvsh =  vleraPaTvsh.reduce(function(index, val) { return index + val; }, 0)
      ...
      {(popupState) => (
        <div>
         ...
         style={{marginTop:'15px'}}
          ...
          >
            <Typography style={{padding:'1rem',display:'grid',flexDirection:'row'}}  >
            <Button style={{color:"#1971c2",cursor:"pointer" ,margin:'5px'}} onClick={handlePrint}> <div style={{fontSize:'12px'}}>Printo Fature Blerje
              </div><PrintSharp style={{blockSize:'17px'}}/> </Button>
            <Button style={{color:"#1971c2",cursor:"pointer" ,margin:'5px'}} > <div style={{fontSize:'12px'}}>Printo Fature Blerje 2
              </div><PrintSharp style={{blockSize:'17px'}}/> </Button>
